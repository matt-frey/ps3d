module impl_rk4_mod
    ...
    contains
        ...
        subroutine impl_rk4_substep_one(self, q, sqs, qdi, qdf, mq)
            ...
            qdi = q
            q = (qdi + dt2 * sqs)
            call fdecomp%field_combine_semi_spectral(q)
            !$omp parallel do private(iz)  default(shared)
            do iz = 0, nz
                q(iz, :, :) = q(iz, :, :) * mq
            enddo
            !$omp end parallel do
            call fdecomp%field_decompose_semi_spectral(q)
        ...
        subroutine impl_rk4_substep_two(self, q, sqs, qdi, qdf, mq, pq)
            ...
            call fdecomp%field_combine_semi_spectral(sqs)
            !$omp parallel do private(iz)  default(shared)
            do iz = 0, nz
                sqs(iz, :, :) = pq * sqs(iz, :, :)
            enddo
            !$omp end parallel do
            call fdecomp%field_decompose_semi_spectral(sqs)
