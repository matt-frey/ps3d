...
program test_implicit_rk
    ...
    do n = 1, 4
        ...
        do nc = 1, 3
            call flayout%field_combine_semi_spectral(ref(:, :, :, nc))
            call flayout%field_combine_semi_spectral(src(:, :, :, nc))
            do iz = 0, nz
                ref(iz, :, :, nc) = ed * ref(iz, :, :, nc) + (one - ed) * src(iz, :, :, nc) / vdiss
            enddo
            call flayout%field_decompose_semi_spectral(ref(:, :, :, nc))
            call flayout%field_decompose_semi_spectral(src(:, :, :, nc))
        enddo
    ...
    contains
        ...
        subroutine impl_rk4(t, dt)
            ...
            svor = (qdi + dt2 * svorts)
            do nc = 1, 3
                call flayout%field_combine_semi_spectral(svor(:, :, :, nc))
                !$omp parallel do private(iz)  default(shared)
                do iz = 0, nz
                    svor(iz, :, :, nc) = svor(iz, :, :, nc) * emq
                enddo
                !$omp end parallel do
                call flayout%field_decompose_semi_spectral(svor(:, :, :, nc))
            enddo
            ...
            do nc = 1, 3
                call flayout%field_combine_semi_spectral(svorts(:, :, :, nc))
                !$omp parallel do private(iz)  default(shared)
                do iz = 0, nz
                    svorts(iz, :, :, nc) = svorts(iz, :, :, nc) * epq
                enddo
                !$omp end parallel do
                call flayout%field_decompose_semi_spectral(svorts(:, :, :, nc))
            enddo
