...
!
!                   This unit test checks the diff2dy subroutine.
! =============================================================================
program test_diffy
    use unit_test
    use constants, only : pi, twopi, f12, zero, four, two
    use sta3dfft, only : initialise_fft, finalise_fft, fft2d, ifft2d, diff2dy
    ...
    do i = box%lo(1), box%hi(1)
        ...
        do j = box%lo(2), box%hi(2)
            y = lower(2) + dble(j) * dx(2)
            passed = (passed .and. (fp(1, j, i) - (-four * dsin(four * y)) < 1.0e-12))
        enddo
    enddo
